...
isZero (Just t) = (value t == Touching 0) && (hidden t == False)
isZero _	= False
...
runGame b =
  case (getGameState b) of
    ...
    Continue -> do
	putStrLn $ show b
        putStrLn "Next turn: Mark/unmark (m) or dig (d)?"
        m <- getLine
        putStrLn "Which row?"
        y <- getLine
...
main = do
  -- Setup
  putStrLn "Height of board?"
  height <- getLine
  putStrLn "Width of board?"
  width <- getLine
  putStrLn "Number of bombs?"
  bombs <- getLine
  rng <- newStdGen
